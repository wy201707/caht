<template>
    <div class="TimeInfoComponent-pannel" v-show="this.$store.getters._getDefaultTimeInfoVisible">
        <div class="TimeInfoComponent-header">
            <!-- <span> timeInfo -大佬给了一个不用后端的方案，但是import和require不能一起混用（反正我报错） </span> -->
            <span>动画参数设置</span>
    ...
    methods: {
        ...
        animate(sDT, eDT) {
            ...
            const frame = (timestamp) => {
                ...
                setTimeout(() => {
                    ...
                    // requestAnimationFrame(frame);  大佬说的应该就是这句
                    frame();
        ...
        choiceType() {
            ...
            switch (this.TimeSetData.DataType) {
                case 1:
                    ...
                    this.steps = [
                        //0412  这个step等统计了数值后，在规定样式                        // { value: -1, color: '#b0e0e6' },
        ...
        onSubmit() {
            ...
            if (this.TimeSetData.startDT) {
                ...
                this.TimeSetData.startDT = new Date('2021/08/15 0:10:00');
            }
        ...
        async getRadarOneData(dateStr) {
            ...
            //file:///C:/inetpub/wwwroot/vuegis/static/data/2021-09-12/
            ...
            const renderer = {
                ...
                symbol: {
                    type: 'simple-marker', // autocasts as new SimpleMarkerSymbol() 换一个啊！
                    // Arrow marker
